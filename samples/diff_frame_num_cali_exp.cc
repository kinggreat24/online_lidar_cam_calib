...
namespace {
    std::queue<int> kFrameNums({ 0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000 });
...
int main(int argc, char const* argv[]) {
    ...
    for (const int& frame_num : kFrameNums) {
        LOG(INFO) << "Start frame number set to: " << frame_num;
        conf.opt_conf.start_frame_num = frame_num;
        ...
        CHECK_LT(start_f_idx + frame_num, kitti_dataset.Size());
        for (int i = start_f_idx; i < start_f_idx + frame_num; ++i) {
        ...
        fnum_err_bag->AddData(frame_num, err);
...
#if 0
    // std::vector<Eigen::Isometry3f> extri_errors = phc::utils::Rand::NormalIsometry3f(0.0f, 2.0f/3.0f, 0.0f, 0.2f/3.0f, 1);
    std::vector<Eigen::Isometry3f> extri_errors = phc::utils::Rand::UniformIsometry3f(2.0f, 0.1f, 1);
    init_extri = extri_errors.front() * origin_extri;

    phc::utils::FrameNumWithFinalErrDataBag::Ptr fnum_err_bag(new phc::utils::FrameNumWithFinalErrDataBag("fnum_err_20"));
    ...
    int start_f_idx = phc::utils::Config::Get<int>("start_frame_idx");
    CHECK_LT(start_f_idx + kFrameNums.back(), kitti_dataset.Size());
    for (int i = start_f_idx; i < kitti_dataset.Size(); ++i) {
        ...
        if (f_num == kFrameNums.front()) {
            ...
            fnum_err_bag->AddData(kFrameNums.front(), err);
    ...
    for (int i = 0; i < extri_errors.size(); ++i) {
        ...
        SingleExtrinsicCompute(init_extri, conf, kitti_dataset, name);
    }
