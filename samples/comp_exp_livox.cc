...
int main(int argc, char const* argv[]) {
    ...
    phc::utils::FrameNumWithFinalErrDataBag::Ptr data_bag(new phc::utils::FrameNumWithFinalErrDataBag("proposed_2"));
    m2d::DataManager::Add(data_bag);
    ...
    for (int i = 0; i < errs.size(); ++i) {
        ...
        for (int f_idx = start_f_idx; f_idx < start_f_idx + f_num; ++f_idx) {
            lvx::Frame f = livox_loader[f_idx];
