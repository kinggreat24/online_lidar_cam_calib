...
int main(int argc, char const* argv[]) {
    ...
    // Load extri pertub file
    std::string pertub_file = phc::utils::Config::Get<std::string>("extri_pertub_file");
    std::vector<Eigen::Isometry3f> file_extri_pertubs = phc::utils::LoadPertubationFile(pertub_file);
    ...
    for (int session_id = 0; session_id < 3; ++session_id) {
        ...
        std::vector<Eigen::Isometry3f> errs = file_extri_pertubs;
