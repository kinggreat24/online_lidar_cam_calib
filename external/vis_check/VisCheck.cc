...
void VisCheck::ComputeVisibility(PtIndices& visible_pts){
    ...
    for(size_t i = 0; i < proj_pts->size(); ++i){
        ...
        if (max_view_range_ > 0.0f) {
            if (ComputeSquaredEuclideanDistToOrigin(cloud_in_cam->at(proj_indices[i])) > max_view_range_ * max_view_range_) {
                continue;
            }
        }
    ...
    // mean_vis_score_ = std::reduce(std::execution::par, vis_scores.begin(), vis_scores.end()) / vis_scores.size();
    mean_vis_score_ = std::accumulate(vis_scores.begin(), vis_scores.end(), 0.0f) / vis_scores.size();
...
void VisCheck::ComputeVisibilityInterestIndices(PtIndices& visible_pts, const PtIndices& interest_indices) {
    // Transform to camera frame
    Pt3dCloud::Ptr cloud_in_cam(new Pt3dCloud());
    pcl::transformPointCloud(*cloud_3d_ptr_, *cloud_in_cam, cam_pose_.matrix());
    ...
    // Use set for faster query
    std::unordered_set<pcl::index_t> inter_indices_uset = PtIndice2Uset(interest_indices);

    std::unordered_map<pcl::index_t, float> ptidx_score_map;

    for (size_t i = 0; i < proj_pts->size(); ++i) {
        if (inter_indices_uset.find(proj_indices[i]) == inter_indices_uset.end()) { // Not interest point
            continue;
        }
        ...
        ptidx_score_map.insert(std::make_pair(proj_indices[i], vis_score));
    }

    for (const auto &kv : ptidx_score_map) {
        if (kv.second > vis_score_thresh_) {
            visible_pts.indices.push_back(kv.first);
        }
    }
}

std::unordered_set<pcl::index_t> VisCheck::PtIndice2Uset(const PtIndices& indices) {
    std::unordered_set<pcl::index_t> result;
...
float VisCheck::ComputeSquaredEuclideanDistToOrigin(const Pt3d& pt) {
...
void VisCheck::ProjectToImageSpace(Pt3dCloud::ConstPtr cloud_3d, Pt2dCloud::Ptr cloud_proj, std::vector<unsigned int>& indice){
    ...
    for(size_t i = 0; i < cloud_3d->size(); ++i){
        ...
        if (proj.x < 0 + discard_edge_size_ || proj.x >= img_width_ - discard_edge_size_ || proj.y < 0 + discard_edge_size_ || proj.y >= img_height_ - discard_edge_size_) { continue; }
